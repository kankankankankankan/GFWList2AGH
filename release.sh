...
function GetData() {
    ...
    gfwlist2agh_modify=(
        "https://raw.githubusercontent.com/kankankankankankan/GFWList2AGH/refs/heads/source/data/data_modify.txt"
    )
...
function GenerateRules() {
    function FileName() {
        ...
        if [ "${software_name}" == "adguardhome" ] || [ "${software_name}" == "adguardhome_new" ] || [ "${software_name}" == "domain" ]; then
            ...
            file_extension="conf"
        elif [ "${software_name}" == "routeros" ]; then
            file_extension="rsc"
        else
            file_extension="dev"
        fi
...
function OutputData() {
    ...
    ## RouterOS
    software_name="routeros" && generate_file="black" && generate_mode="lite" && GenerateRules
    ## SmartDNS
